...
func (c *FriendController) SendFriendRequest(ctx *fiber.Ctx) error {
	...
	if err := c.notificationService.SendWebSocketNotification(request.ReceiverId, "New Friend Request", "You have a new friend request!"); err != nil {
...
func (c *FriendController) AcceptFriendRequest(ctx *fiber.Ctx) error {
	...
	// Send notification via WebSocket
	if err := c.notificationService.SendWebSocketNotification(request.SenderId, "Friend Request Accepted", "Your friend request has been accepted!"); err != nil {
		log.Printf("Failed to send notification: %v", err)
		return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
	}
