...
func NewMatchPlayersController(matchPlayersService *services.MatchPlayersService, authService *services.AuthService, db *gorm.DB, openAIService *services.OpenAIService) *MatchPlayersController {
...
func (ctrl *MatchPlayersController) GetMatchPlayersByMatchIDHandler(c *fiber.Ctx) error {
	...
	// Suggest formations based on player statistics using OpenAI
	formations, err := ctrl.OpenAIService.SuggestFormations(players)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Failed to get formations from OpenAI"})
	}

	// Return response with player statistics and suggested formations
